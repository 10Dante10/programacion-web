...
const CurrencyConverter = () => {
  ...
  useEffect(() => {
    fetch(
      "https://v6.exchangerate-api.com/v6/26df701c9053af33ff4ec9b9/latest/USD"
    )
  ...
  useEffect(() => {
    if (fromCurrency != null && toCurrency != null) {
      fetch(
        `https://v6.exchangerate-api.com/v6/26df701c9053af33ff4ec9b9/latest/${fromCurrency}`
      )
        .then((response) => response.json())
  ...
  return (
    <div>
      ...
      <h2>
        {amount} {fromCurrency} is equivalent to {convertedAmount.toFixed(2)}{" "}
        {toCurrency}
      </h2>
    </div>
  );
